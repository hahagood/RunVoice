...
fun AboutScreen(onBack: () -> Unit) {
    Column(
        ...
        Column(
            ...
            SectionBody(
                "1. 首次使用请授予定位、蓝牙、通知权限\n" +
                "2. 点击右上角「心率监控」可搜索并连接 BLE 心率设备\n" +
                "3. 点击「开始跑步」启动 GPS 追踪和语音播报\n" +
                "4. 跑步过程中可点击「停止跑步」暂停，再选择继续或结束\n" +
                "5. 节拍器可在跑步前或跑步中随时开关和调整 BPM\n" +
                "6. 若需完整重启 APP，可暂停后选择「暂存本次跑步并退出」；重新打开后选择继续上次跑步\n" +
                "7. 若进程意外终止或手机断电，重新打开 APP 后同样可以续跑；停止期间不计时间和距离"
            )
            ...
            SectionBody(
                ...
                "跑步期间，临时 CSV 保存在 App 专属目录。结束跑步后点击「保存海报和轨迹」，会同时保存数据摘要海报，" +
                "并将轨迹复制到 Documents/RunVoice/gps-traces；该公共副本不会随卸载自动删除。" +
                "直接返回首页会删除未保存的临时轨迹。\n\n" +
