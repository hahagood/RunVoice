...
- **定位异常自恢复** — 持续高速或 GPS 跳点会进入隔离定位链，确认后安全重锚，避免坏点过滤永久锁死；异常和恢复都会通过耳机语音提示
- **闭环分段播报** — 自动从历史轨迹发现局部闭环入口并锁定固定圈锚点；离开后再次返回时播报上一段的平均配速与距离，避免小圈多次复跑时因每圈重设锚点产生累计误差
...
- **结束页截图保存** — 结束确认页支持将本次 GPS 航迹与结束日期时间、距离、时间、平均配速和最大心率套印为本地海报版图片；闭环复跑会异色并逐层抬升绘制，往返跑的去程与回程会拆为相邻两层，连续多圈后的退出线会保持在新的独立层；轨迹使用独立配色完整绘制，数据文字通过自适应宽度的半透明玻璃衬底保持清晰，若轨迹点不足则自动降级为纯摘要图片
...
> "第一段完成，平均配速5分18秒每公里，距离800米。第二段开始"
...
- 套圈入口不要求等于整次活动的起点；首次确认局部闭环后会固定历史入口，后续圈只更新时间和距离基准，不再逐圈移动空间锚点
...
│   ├── LapDetector.kt          # 局部闭环发现、固定圈锚点与虚拟计时门
...
- 顶部日期和时间居中显示
- 摘要区统一展示：距离、时间、平均配速、最大心率；配速和心率不重复显示单位，以减少轨迹遮挡
- 轨迹先完整绘制，数据文字最后绘制；每段文字背后的半透明圆角衬底按实际文字宽度自适应，只压暗必要区域
- 首次确认局部闭环后固定历史圈锚点，后续圈不会把提前触发的当前位置设为新起点，因此小圈多次复跑不会产生逐圈累计误差
