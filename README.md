...
3. （可选）点击节拍器 BPM 数字开启步频节拍，◀ ▶ 调整 BPM（开关状态自动记忆）
4. 点击"开始跑步"，放入口袋，戴上蓝牙耳机
5. 每跑完 1 公里自动语音播报
6. 跑完后点"停止跑步" → "结束跑步"
7. 点击顶部"RunVoice"可查看关于页面和免责声明
...
将每次跑步数据上传云端保存，便于回顾训练记录：

- **方案** — Cloudflare Workers + D1（SQLite），免费额度够个人使用
- **数据** — 日期、距离、用时、平均配速、平均/最高心率等
- **触发** — 结束跑步时自动上传
- **查询** — 后续可加 Web 页面或 APP 内历史记录
