...
- **闭环分段播报** — 离开起点并完成闭环返回后，自动播报上一段的平均配速与距离，并提示下一段开始
...
- **结束页截图保存** — 结束确认页支持将本次 GPS 航迹与结束日期时间、距离、总用时、平均配速和最大心率套印为本地海报版图片；闭环复跑会异色并逐层抬升绘制，往返跑的去程与回程会拆为相邻两层，连续多圈后的退出线会保持在新的独立层；轨迹使用独立于白/黄/红正文和蓝色背景的专用配色，并作为海报最上层绘制，若轨迹点不足则自动降级为纯摘要图片
