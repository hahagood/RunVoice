...
- **结束页截图保存** — 结束确认页支持将结束时间、距离、用时、平均配速和最大心率保存为本地图片，便于后续自行分享
...
4. **保存数据** / **放弃本次** / **保存截图**（结束确认页）→ 保留本次记录 / 删除本次记录 / 保存结束摘要图片到本地
...
7. 跑完后点"暂停跑步" → "结束本次"
8. 结束确认页顶部显示本次结束时间，并展示距离、用时、平均配速和最大心率
9. 可选择"保存数据"、"放弃本次"或"保存截图"
