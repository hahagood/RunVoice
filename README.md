...
- **耳机单击即时播报** — 跑步中可尝试通过蓝牙耳机的单击媒体键触发一次当前数据播报
...
3. **继续跑步** / **结束本次**（两个按钮）→ 恢复跑步 或 结束本次
4. **耳机单击媒体键**（设备支持时）→ 立即播报当前距离、时间、当前心率、最大心率和配速
...
7. 跑完后点"暂停跑步" → "结束本次"
8. 点击顶部"RunVoice"可查看关于页面和免责声明

## GPS 轨迹调试

为了分析 RunVoice 与其他跑步 APP 的距离差异，每次跑步会自动保存一份 GPS 轨迹 CSV。
