...
- **静止漂移过滤** — 加速度传感器结合 GPS 持续移动确认；静止时过滤 GPS 漂移，传感器误判时可在 GPS 连续移动后补回距离
...
- **静止判定提醒** — 连续判定静止 5 秒后语音提示“检测为静止”，帮助及时发现真正停下或传感器误判
...
- 当连续判定静止 5 秒后，会播报“检测为静止”；同一段静止/误判期间最多每 60 秒提醒一次
