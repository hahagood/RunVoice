...
4. **保存数据** / **放弃本次** / **继续跑步**（结束确认页）→ 保留本次记录 / 删除本次记录 / 返回跑步页
...
7. 跑完后点"暂停跑步" → "结束本次"
8. 在结束确认页选择"保存数据"或"放弃本次"
9. 点击顶部"RunVoice"可查看关于页面和免责声明

## GPS 轨迹调试

为了分析 RunVoice 与其他跑步 APP 的距离差异，每次跑步会生成一份 GPS 轨迹 CSV。

- 结束时选择 **保存数据**：保留本次 GPS 轨迹 CSV
- 结束时选择 **放弃本次**：删除本次 GPS 轨迹 CSV
...
当前版本结束跑步后的“保存数据 / 放弃本次”，实际控制的是本次会话及 GPS 轨迹文件是否保留。
...
- **保存每次跑步数据** — 先把单次跑步摘要稳定落盘
- **历史记录与汇总页** — 再做最近记录、周/月跑量、最大心率趋势
